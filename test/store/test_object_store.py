...
def test_minio_get_artifact_url(mocked_object_store, general_uuid):
    ...
    assert result == f'minio.test:1000/minio_test_bucket/{general_uuid}/{general_uuid}_test_file.tiff'


def test_get_icon_url(mocked_object_store):
    result = mocked_object_store.get_icon_url(plugin_id='test_plugin')
    assert result == 'minio.test:1000/minio_test_bucket/assets/test_plugin/latest/ICON.jpeg'
