...
These are settings that plugins using climatoology will need to provide as environment variables or in a file called
`.env.base` in the current working directory on call of `climatoology.app.plugin.start_plugin`.

Plugins should come with a prefilled `.env.base_template` file that contains some development defaults to work with a
